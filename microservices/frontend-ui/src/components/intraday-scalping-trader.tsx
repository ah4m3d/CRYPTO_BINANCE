...
export default function IntradayScalpingTrader({ tradingState, isEnabled, onToggle }: IntradayScalpingTraderProps) {
  // Trading state
  ...
  const closePosition = (symbolToClose: string, reason: string = 'Manual Close') => {
    ...
    // Also close position on backend if auto trading is enabled
    if (isEnabled) {
      closePositionOnBackend(symbolToClose)
    }
  ...
  // Backend API functions for watchlist integration
  const fetchWatchlistFromBackend = useCallback(async () => {
    try {
      console.log('🔍 Fetching market data from Trading Engine')
      const response = await fetch(`${TRADING_ENGINE_URL}/api/market-data`)
      if (!response.ok) {
      ...
      const stocksData = marketData.map((item: any) => ({
        symbol: item.symbol.replace('USDT', '-USD'), // Convert BTCUSDT to BTC-USD format
        name: item.name || item.symbol,
        price: item.price || 0,
        change: item.change || 0,
        changePercent: item.changePercent || 0,
        technicalAnalysis: item.technical,
        isLoading: false,
        error: undefined
      }))
  ...
  const addToBackendWatchlist = useCallback(async (symbol: string) => {
    try {
      // Note: Backend doesn't support dynamic watchlist management yet
      // This is a placeholder for future implementation
      console.log(`📝 Note: Adding ${symbol} to watchlist (not implemented in backend yet)`)
      return true
    } catch (error) {
      console.error(`❌ Error adding ${symbol} to backend watchlist:`, error)
      return false
    }
  }, [])

  const removeFromBackendWatchlist = useCallback(async (symbol: string) => {
    try {
      // Note: Backend doesn't support dynamic watchlist management yet
      // This is a placeholder for future implementation
      console.log(`📝 Note: Removing ${symbol} from watchlist (not implemented in backend yet)`)
      return true
    } catch (error) {
      console.error(`❌ Error removing ${symbol} from backend watchlist:`, error)
      return false
    }
  }, [])
  ...
  // Sync with trading state from props
  useEffect(() => {
    if (tradingState) {
      console.log('� Syncing frontend state with backend trading state')
      
      // Update trades
      if (tradingState.trades) {
        setTrades(tradingState.trades)
      }
      
      // Update positions
      if (tradingState.positions) {
        setPositions(tradingState.positions)
      }
      
      // Update balances
      if (tradingState.tradingBalance !== undefined) {
        setTradingBalance(tradingState.tradingBalance)
      }
      
      if (tradingState.availableBalance !== undefined) {
        setAvailableBalance(tradingState.availableBalance)
      }
      
      // Update P&L
      if (tradingState.totalPnL !== undefined) {
        setTotalPnL(tradingState.totalPnL)
      }
      
      if (tradingState.dayPnL !== undefined) {
        setDayPnL(tradingState.dayPnL)
      }
      
      // Update settings
      if (tradingState.settings) {
        const settings = tradingState.settings
        setMinConfidence(settings.minConfidence || 20)
        setMaxPositionSize(settings.maxPositionSize || 10000)
        setRiskPerTrade(settings.riskPerTrade || 2)
        setStopLossPercent(settings.stopLossPercent || 0.5)
        setTakeProfitPercent(settings.takeProfitPercent || 0.7)
      }
      
      // Update watchlist from backend data
      if (tradingState.watchlist) {
        const watchlistData = tradingState.watchlist.map((item: any) => ({
  ...
  // Fetch trading state from backend (legacy function - now replaced by props)
  const fetchTradingStateFromBackend = useCallback(async () => {
    try {
      console.log('🔍 Fetching trading state from Trading Engine')
      const response = await fetch(`${TRADING_ENGINE_URL}/api/trading-state`)
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`)
      }
      const backendTradingState = await response.json()
      console.log('✅ Trading state from backend:', backendTradingState)
      
      return backendTradingState
    } catch (error) {
      console.error('❌ Error fetching trading state from backend:', error)
      return null
    }
  }, [])
    } catch (error) {
  ...
  // Toggle auto trading via backend API
  const toggleAutoTradingOnBackend = useCallback(async (enabled: boolean) => {
    try {
      console.log(`🔄 ${enabled ? 'Enabling' : 'Disabling'} auto trading on backend`)
      ...
      console.log(`✅ Auto trading ${enabled ? 'enabled' : 'disabled'} on backend:`, result)
      
      // Refresh trading state to get updated settings
      setTimeout(() => {
        fetchTradingStateFromBackend()
      }, 1000)
      
      return result
    } catch (error) {
      console.error('❌ Error toggling auto trading on backend:', error)
      return null
    }
  }, [])

  // Close position via backend API
  const closePositionOnBackend = useCallback(async (symbol: string) => {
    try {
      console.log(`🔄 Closing position ${symbol} on backend`)
      const response = await fetch(`${TRADING_ENGINE_URL}/api/trading/close/${symbol}`, {
        method: 'POST'
      })
      ...
      console.log(`✅ Position ${symbol} closed on backend`)
      
      // Refresh trading state to get updated positions and trades
      setTimeout(() => {
        fetchTradingStateFromBackend()
      }, 1000)
      ...
      console.error(`❌ Error closing position ${symbol} on backend:`, error)
      return false
    }
  }, [])

  // Initialize watchlist from backend
  useEffect(() => {
    const initializeWatchlist = async () => {
      setIsWatchlistLoading(true)
      
      console.log('🔄 Loading watchlist from Trading Engine backend')
      
      // Try to fetch from backend first
      const backendWatchlist = await fetchWatchlistFromBackend()
      
      if (backendWatchlist.length > 0) {
        console.log(`✅ Loaded ${backendWatchlist.length} symbols from backend watchlist`)
        setWatchlistStocks(backendWatchlist)
      } else {
        console.log('⚠️ No backend watchlist found, using default crypto symbols')
        // Fallback to default symbols if backend is not available
        const defaultSymbols = ['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'XRPUSDT', 'ADAUSDT']
        const stocksData = defaultSymbols.map(symbol => ({
          symbol,
          name: symbol,
          price: 0,
          change: 0,
  ...
  const addCustomStock = async () => {
    ...
    try {
      ...
      await addToBackendWatchlist(symbol)
      
      // Add to local state with loading indicator
      ...
      // Fetch current data for the new symbol
      ...
      // Add to local state anyway with error indicator
      setWatchlistStocks(prev => [...prev, {
        symbol,
        name: symbol,
        price: 0,
        ...
        changePercent: 0,
        isLoading: false,
        error: 'Failed to add to backend'
      }])
    }
  ...
  const removeStock = async (symbol: string) => {
    try {
      ...
      await removeFromBackendWatchlist(symbol)
      
      // Remove from local state
      setWatchlistStocks(prev => prev.filter(stock => stock.symbol !== symbol))
    } catch (error) {
      console.error('Failed to remove symbol from backend watchlist:', error)
  ...
  // Fetch trading state from backend on component mount and periodically
  useEffect(() => {
    // Initial fetch
    fetchTradingStateFromBackend()
    
    // Set up interval to fetch trading state every 5 seconds for real-time updates
    const interval = setInterval(() => {
      fetchTradingStateFromBackend()
    }, 5000) // 5 seconds for real-time position updates
              ...
              <div className="flex items-center gap-3">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    ...
                    toggleAutoTradingOnBackend(checked)
                  }}
                />
              </div>
            </div>

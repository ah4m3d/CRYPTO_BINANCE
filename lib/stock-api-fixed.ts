...
class YahooFinanceAPI {
  ...
  async getTechnicalAnalysis(symbol: string, timeframe: string = '1d', days: number = 30): Promise<TechnicalIndicators> {
    try {
      ...
      const response = await fetch(`${this.baseURL}?symbols=${encodeURIComponent(symbol)}&timeframe=${timeframe}&days=${days}`)
      ...
      const technicalData: TechnicalData[] = stockData.historicalData?.map((item: any) => ({
        date: item.date,
        open: item.open,
        high: item.high,
        low: item.low,
  ...
  async getBatchTechnicalAnalysis(symbols: string[], timeframe: string = '1d', days: number = 30): Promise<Map<string, TechnicalIndicators>> {
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        technicalMap.set(symbols[index], result.value)
      } else {
        console.error(`Failed to get technical analysis for ${symbols[index]}:`, result.reason)
        // Set default values for failed requests
        technicalMap.set(symbols[index], {
          vwap: 0,
          rsi: 50,
          buyPrice: 0,
          sellPrice: 0,
  ...
  async getHistoricalData(symbol: string, period: string = '1mo', interval: string = '1d'): Promise<HistoricalData> {
    try {
      const response = await fetch(`${this.baseURL}?symbols=${encodeURIComponent(symbol)}&period=${period}&interval=${interval}`)
